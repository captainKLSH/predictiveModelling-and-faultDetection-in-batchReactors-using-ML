...
def runa(file2):
    ...
    df['fault_type']=0
    fault_times=[46, 92, 112, 145, 198, 234]
    window=2
    fault_condition = False
    for t in fault_times:
...
def main():
    file1 = '../Transferlearning/agitatorfault.csv'
    file2='../Transferlearning/runawayfault.csv'
    # Call agitator fault function
    df_agi = agi(file1)
    df_agi.to_csv('../Transferlearning/agitatorfault_L.csv', index=False)

    # Call run fault function
    df_run = runa(file2)
    df_run.to_csv('../Transferlearning/runawayfault_L.csv', index=False)
