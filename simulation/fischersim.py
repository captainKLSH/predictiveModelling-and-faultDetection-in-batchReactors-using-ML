...
def run_reactor_simulation_final(fault_type='none', fault_time=80, fault_magnitude=0.5, fault_duration=0):
    """
    Simulates a semibatch reactor for the esterification of acetic acid with ethanol
    to produce ethyl acetate, with gradual fault introduction.

    Args:
        fault_type (str): The type of fault to introduce.
        fault_time (int): The time step (minutes) at which a fault begins.
        fault_magnitude (float): The final severity of the fault.
        fault_duration (int): The time in minutes over which the fault ramps up to its full magnitude.
                              If 0, the fault is instantaneous.
    Returns:
        pandas.DataFrame: A DataFrame containing the time-series data of the simulation.
    """
    # 1. --- Constants and Initial Conditions for Fischer Esterification ---

    # Reaction properties for Acetic Acid + Ethanol -> Ethyl Acetate + Water
    Ea = 5.5e4      # Activation Energy (J/mol) - Typical for esterification
    R = 8.314       # Gas Constant (J/mol·K)
    k0 = 1.2e4      # Pre-exponential factor (m^3/mol·s) - Adjusted for this reaction
    deltaH = -2.0e4 # Heat of reaction (J/mol) - Mildly exothermic
    ...
    MW_B = 60.05      # Acetic Acid (C2H4O2) - Initial charge
    MW_A = 46.07      # Ethanol (C2H5OH) - Feed
    MW_P = 88.11      # Ethyl Acetate (C4H8O2) - Product
    MW_H2O = 18.015   # Water (H2O) - Product

    # Ethanol Feed Properties
    ethanol_density = 789 # kg/m^3
    Cp_feed = 2460      # Heat capacity of ethanol feed (J/kg·K)
    T_feed = 298.15     # Feed temperature (25°C)
    C_A_feed = (ethanol_density / MW_A) * 1000 # Concentration of pure ethanol feed

    # Dynamic Property Parameters for Mixture
    rho_initial = 1049   # Initial density (Acetic Acid) (kg/m^3)
    Cp_initial = 2040    # Initial heat capacity (Acetic Acid) (J/kg·K)
    rho_final = 850      # Estimated final mixture density (kg/m^3)
    Cp_final = 2200      # Estimated final mixture heat capacity (J/kg·K)

    # Reactor and Acetic Acid Load
    moles_B_initial_kmol = 120.0 # Starting with acetic acid
    ...
    for i in range(1, n_steps + 1):
        ...
        # Define Temperature Setpoint Profile (Ramp from T0 to 75C)
        if current_time_min < feed_duration_min:
            T_setpoint[i] = T0 + (348.15 - T0) * (current_time_min / feed_duration_min)
        else:
            T_setpoint[i] = 348.15
        ...
        is_saturated_high = (controller_output >= Coolant_Flow_max) and (error < 0)
        is_saturated_low = (controller_output <= 0) and (error > 0)
        ...
        # --- Gradual Fault Logic ---
        current_fault_magnitude = 0.0
        if current_time_min >= fault_time:
            if fault_duration > 0:
                elapsed_fault_time = current_time_min - fault_time
                ramp_fraction = min(1.0, elapsed_fault_time / fault_duration)
                current_fault_magnitude = fault_magnitude * ramp_fraction
            else:
                current_fault_magnitude = fault_magnitude

        # --- Fault Application and Noise Injection Logic ---
        ...
        if fault_type == 'cooling_failure': 
            coolant_flow_setpoint *= (1 - current_fault_magnitude)
        elif fault_type == 'agitator_failure': 
            agitator_speed_setpoint *= (1 - current_fault_magnitude)
        elif fault_type == 'feed_rate_error' and current_time_min < feed_duration_min: 
            feed_flow_setpoint *= (1 + current_fault_magnitude)
        ...
        Q_feed = Feed_Flow[i] * ethanol_density * Cp_feed * (T_feed - T_prev)
    ...
    results = pd.DataFrame({
        ...
        'AceticAcid_Concentration_mol_m3': C_B, 'Ethanol_Concentration_mol_m3': C_A,
...
if __name__ == '__main__':
    ...
    fig.suptitle('Semibatch Ethyl Acetate Esterification Simulation', fontsize=16)
    ...
    # Concentration Plot
    axs[0, 1].plot(normal_data['Time_min'], normal_data['AceticAcid_Concentration_mol_m3'], label='Acetic Acid')
    axs[0, 1].plot(normal_data['Time_min'], normal_data['Ethanol_Concentration_mol_m3'], label='Ethanol')
    ...
    axs[3, 0].set_title('Ethanol Feed Flow'); axs[3, 0].set_ylabel('Flow (m³/s)'); axs[3, 0].grid(True)
